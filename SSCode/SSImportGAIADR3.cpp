...
// GAIA stars are read into a vector of SSObjects (stars).
// GAIA stars brighter than the Johnson V magnitude minimum (vmin) or fainter than the
// Johnson V magnitude maximum (vmax) are discarded, unless thay have HIP or TYC identifiers.
...
int SSImportGAIA17 ( const string &filename, SSObjectArray &stars, float vmin, float vmax )
{
    ...
    while ( true )
    {
        ...
        // Convert GAIA magnitudes to Johnson B and V if both GAIA BP and RP colors are valid.
        ...
        // Discard stars outside than V magnitude limits unless they have HIP/TYC identifiers
        
        if ( ( vmag < vmin || vmag > vmax ) && gaia.hip_source_id == 0 && gaia.tyc_source_id == 0 )
            continue;
        ...
        SSIdentifier tyc = gaia.tyc_source_id ? SSIdentifier ( kCatTYC, gaia.tyc_source_id ) : SSIdentifier();
        SSIdentifier hip = gaia.hip_source_id ? SSIdentifier ( kCatHIP, gaia.hip_source_id ) : SSIdentifier();

        // Get spectral type implied from temperature
        
        int spectype = SSStar::spectralType ( gaia.teff_k );
        string spectrum = SSStar::formatSpectrum ( spectype, -1 );
        ...
        if ( ( hip || tyc ) && ( hipMap.size() || tycMap.size() ) )
        {
            ...
            // Copy GAIA magnitudes converted to Johnson
            ...
            // Copy GAIA spectral type to Tycho star if it's missing one.
            
            if ( pTycStar->getSpectralType().empty() )
                pTycStar->setSpectralType ( spectrum );
